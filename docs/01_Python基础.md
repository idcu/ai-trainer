...
# 14. 数字格式化（详细示例）
# 下面介绍几种常用的数字格式化方法，非常实用！

# === 14.1 保留小数位 ===
# 场景：计算结果有很多小数，我们只需要保留几位
pi = 3.1415926535  # 圆周率

# :.2f 表示保留2位小数（f代表浮点数float）
print(f"保留2位小数：{pi:.2f}")
# 输出：保留2位小数：3.14

# :.4f 表示保留4位小数
print(f"保留4位小数：{pi:.4f}")
# 输出：保留4位小数：3.1416  （注意：第5位是9，会四舍五入！）

# === 14.2 百分比显示 ===
# 场景：考试正确率、完成进度等，需要显示成百分比
score = 0.85  # 表示85%的正确率
...
# 再举个例子
discount = 0.15  # 15%的折扣
print(f"折扣：{discount:.0%}")
# 输出：折扣：15%

# === 14.3 千分位分隔符 ===
# 场景：显示大金额、大数据时，加上逗号更易读
money = 1234567.89  # 一百二十三万四千五百六十七点八九

# :,.2f 表示：加千分位逗号，保留2位小数
print(f"金额：{money:,.2f}")
# 输出：金额：1,234,567.89  （看！加了逗号是不是清楚多了？）

# :,.0f 表示：加千分位逗号，不保留小数
population = 1400000000  # 14亿人口
print(f"人口：{population:,.0f}")
# 输出：人口：1,400,000,000

# === 14.4 组合使用 ===
# 可以同时使用多个格式化功能
big_money = 9876543.21
print(f"总金额：{big_money:,.2f}")
# 输出：总金额：9,876,543.21

# === 小技巧：记住这些符号的意思 ===
# :    开始格式化
# ,    加千分位逗号
# .2   保留2位小数（.后面的数字可以改，想保留几位就写几）
# f    表示浮点数（小数）
# %    表示显示成百分比
```
